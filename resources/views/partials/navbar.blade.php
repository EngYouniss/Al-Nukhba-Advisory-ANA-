<nav class="navbar navbar-expand-lg navbar-light main-nav px-4 px-lg-5 py-3 py-lg-0" dir="rtl" aria-label="التنقل الرئيسي">
  <div class="container-xxl">
    ...
    <a href="{{ route('client.index') }}" class="navbar-brand p-0 d-flex align-items-center gap-2">
      ...
      <h1 class="m-0 fw-semibold brand-text">مكتب الاستشارات</h1>
    </a>

    <!-- زر القائمة على الموبايل -->
    <button class="navbar-toggler border-0" type="button" data-bs-toggle="collapse" data-bs-target="#navbarCollapse"
            aria-controls="navbarCollapse" aria-expanded="false" aria-label="تبديل القائمة">
    ...
    <div class="collapse navbar-collapse" id="navbarCollapse">
      ...
      <div class="navbar-nav ms-lg-auto py-0">
        ...
        <a href="{{ route('client.index') }}" class="nav-item nav-link @yield('active-contact')">اتصل بنا</a>
      </div>
...
.main-nav .brand-text{ color:#fff; }

/* روابط */
.main-nav .nav-link{
  position: relative;
  color:#fff; padding: 1.25rem .25rem; margin-inline: .85rem;
  font-weight:500; transition: color .2s ease;
}
.main-nav .nav-link:hover,
.main-nav .nav-link:focus{ color:#fff; }
.main-nav .nav-link:focus-visible{ outline:2px solid #fff; outline-offset:2px; border-radius:.25rem; }

/* شريط سفلي للحالة النشطة/الهوفر */
.main-nav .nav-link::before{
  content:""; position:absolute; inset:auto 0 10px 0; height:2px; background:#fff;
  transform: scaleX(0); transform-origin: center; transition: transform .25s ease, bottom .25s ease;
  opacity:.95;
}
.main-nav .nav-link:hover::before,
.main-nav .nav-link.active::before,
.main-nav .nav-link.@yield('active-index')::before{ transform: scaleX(1); bottom:-2px; }

/* عند التثبيت (بعد الهيرو) */
.navbar-stuck{
  position: fixed; background:#fff; border-bottom-color: rgba(16,24,40,.08);
  box-shadow: 0 8px 24px rgba(16,24,40,.08);
}
.navbar-stuck .brand-text{ color: var(--primary); }
.navbar-stuck .nav-link{ color: var(--dark); }
.navbar-stuck .nav-link:hover,
.navbar-stuck .nav-link:focus{ color: var(--primary); }
.navbar-stuck .nav-link::before{ background: var(--primary); }

/* Dropdown (إن استخدمت الدروبداون أعلاه) */
@media (min-width: 992px){
  .main-nav .dropdown-menu{
    display: block; visibility: hidden; opacity:0; transform: translateY(6px);
    transition: .2s ease; border:0; background:#fff;
  }
...
@media (max-width: 991.98px){
  .main-nav{ position: relative; background:#fff; border-bottom-color: rgba(16,24,40,.08); }
  .main-nav .brand-text{ color: var(--primary); }
  .main-nav .nav-link{ color: var(--dark); padding: .75rem 0; margin-inline: 0; }
  .main-nav .nav-link::before{ background: var(--primary); bottom: 0; }
}
